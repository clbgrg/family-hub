...
9. **Deferred from the June 2026 feature batch (branch `feat/areas-history-scheduling`).** Two pieces were intentionally not built and need a decision: (a) **Dashboard configuration** — per-device show/hide toggles for dashboard sections (a localStorage preference, like the existing "students only" toggle). (b) **Message attachments** — files on board posts; deferred because board notes auto-expire after 7 days, so an attachment would vanish with the note. Decide: attach to ephemeral notes anyway, attach only to the new Documents library, or skip.
