...
export default defineNuxtPlugin(async () => {
  ...
  try {
    ...
    if (error.value) {
      throw new Error(
        `Failed to fetch timezone data: ${error.value.statusCode || "Unknown error"}`,
      );
    }
    ...
    const timezoneComponent = new ical.Component(
      ical.parse(vtimezoneBlock.value as string),
    );
    ...
    consola.debug(
      "AppInit: Successfully registered timezone:",
      browserTimezone,
      "on",
  ...
  consola.debug(
    `AppInit: Registered ${integrationConfigs.length} integrations`,
  );
  ...
  try {
    ...
    const [
      _localCalendarResult,
      _localTodosResult,
      _localShoppingResult,
      _todoColumnsResult,
    ] = await Promise.all([
      useAsyncData(
        "calendar-events",
    ...
    if (integrationsResult.data.value) {
      ...
      for (const integration of enabledIntegrations) {
        consola.debug(
          `AppInit: Processing integration: ${integration.name} (${integration.type})`,
        );
        ...
        try {
          ...
          if (!service) {
            consola.warn(
              `AppInit: Failed to create service for ${integration.name}`,
            );
            continue;
          }
          ...
          consola.debug(
            `AppInit: Service initialized and stored for ${integration.name}`,
          );

          if (integration.type === "calendar") {
            integrationDataPromises.push(
              useAsyncData(
                `calendar-events-${integration.id}`,
                async () => {
                  try {
                    const events = await (
    ...
    if (integrationDataPromises.length > 0) {
      consola.debug(
        `AppInit: Loading data for ${integrationDataPromises.length} integrations...`,
      );
